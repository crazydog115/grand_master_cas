...
GrandMasterCas.prototype.bouncer = function(req, res, next){
    var service = encodeURIComponent(req.protocol + '://' + req.get('host') + req.originalUrl);
...
GrandMasterCas.prototype.handleTicket = function( req, res ){
    var ticket = req.query.ticket;
    var sessionName = this.sessionName;
    var ticketArgPos = Math.max(req.originalUrl.indexOf('?ticket'), req.originalUrl.indexOf('&ticket'));
    this.redirectTo = req.protocol + '://' + req.get('host') + req.originalUrl.substring(0, ticketArgPos);
    var service = encodeURIComponent(this.redirectTo);
    var path = this.casPath + '/validate?ticket=' + ticket + '&service=' + service;
    ...
    var request = https.request({
        ...
        response.on('end', function(){
            ...
            if (results[0] === 'yes') {
                ...
                res.send(200, '<html><head><noscript><meta http-equiv="refresh" content="0; url='+that.redirectTo+'" /></noscript></head><body onload="window.location = \''+that.redirectTo+'\'"></html>');
            }
